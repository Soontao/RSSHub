...
const calculateValue = () => {
  ...
  value = {
    ...
    cache: {
      ...
      type:
        envs.CACHE_TYPE === undefined
          ? "memory"
          : envs.CACHE_TYPE.toLowerCase(),
      // 路由缓存时间，单位为秒
      routeExpire: parseInt(envs.CACHE_EXPIRE, 10) || 1 * 60,
      // 不变内容缓存时间，单位为秒
      contentExpire:
        parseInt(envs.CACHE_CONTENT_EXPIRE, 10) || 15 * 24 * 60 * 60,
    },
    ...
    disallowRobot:
      envs.DISALLOW_ROBOT !== "0" && envs.DISALLOW_ROBOT !== "false",
    ...
    s3Storage: {
      ...
      forceFlushInterval: parseInt(
        envs.S3_FORCE_FLUSH_INTERVAL_IN_SECS ?? 1000 * 60 * 60 * 12,
        10
      ),
      forceFlushSize: parseInt(envs.S3_FORCE_FLUSH_ENTRY_SIZE ?? 1000, 10),
    },
  };
};
