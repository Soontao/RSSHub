 ...
 *  language?:string, // https://www.rssboard.org/rss-language-codes
 * }} options
 * @returns
 */
  ...
  const handler = async (ctx) => {
    ...
    if (options.linkExtractor && (options.contentExtractor || options.jsonExtractor)) {
      ...
      ctx.state.data.item = items
        .filter(
          (item) =>
            item.value !== undefined && moment(item.value?.pubDate).isAfter(moment().subtract(config.value.maxOldItemPubDateInDays, "day"))
        )
        .map((item) => item.value);
    }
