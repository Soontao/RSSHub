...
function domAElementLinkExtractor(selector) {
  return ($) =>
    $(selector)
      .get()
...
function createGenericEndpoint(options) {
  ...
  const handler = async (ctx) => {
    ...
    const $ = cheerio.load(await fetchText(options.entryUrl));
    const title = $("title").text();
    const links = options.linkExtractor($);
      ...
      links.map((link) =>
        sem.use(() =>
          ctx.cache.tryGet(link, () =>
            fetchText(link).then((content) => ({ link, guid: md5(link), ...options.contentExtractor(cheerio.load(content)) }))
          )
        )
      )
    );
