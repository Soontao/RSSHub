...
module.exports = async (ctx, next) => {
  if (ctx.request.path.match(unsupportedRegrx)) {
    throw Error(
      '<b>JSON output had been removed, see: <a href="https://github.com/DIYgod/RSSHub/issues/1114">https://github.com/DIYgod/RSSHub/issues/1114</a></b>',
    );
  }
          ...
          if (item.enclosure_length) {
            const itunes_duration =
              ...
              (Math.floor((item.enclosure_length % 3600) / 60) / 100)
                ...
                .slice(-2) +
              ":" +
              (((item.enclosure_length % 3600) % 60) / 100)
                .toFixed(2)
                .slice(-2);
