...
const { Mutex } = require("@newdash/newdash/functional/Mutex");
...
class Cache {
    ...
    return lock.use(async () => {
      ...
      if (!v) {
        ...
        if (retrieveDuration > config.longWarningThreshold * 1000) {
          logger.warn(
            `retrieve time too long [${retrieveDuration}] milliseconds: [${key}]`
          );
        } else {
          logger.debug(
            `retrieve time [${retrieveDuration}] milliseconds: [${key}]`
          );
        }
  ...
  set(key, value, maxAge) {
    logger.debug(`set cache [age:${maxAge} seconds]: [${key}]`);
    return null;
  }
