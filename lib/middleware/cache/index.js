...
module.exports = function (app, options = {}) {
  ...
  async function loadLongTimeCache(key, partialItems) {
    ...
    remove(
      partialItems,
      (item) =>
        (item.link && cachedItemsIds.includes(item.link)) ||
        (item.guid && cachedItemsIds.includes(item.guid))
    );
  ...
  async function saveRouteCache(key, ctx) {
    if (
      ctx.response.get("Cache-Control") !== "no-cache" &&
      ctx.state &&
      ctx.state.data
    ) {
  ...
  return async function cacheMiddleWare(ctx, next) {
    ...
    if (ctx.state?.data?.item !== undefined) {
      // uniq by link
      const item = (ctx.state.data.item = uniqBy(
        ctx.state?.data?.item,
        "link"
      ));
