...
async function get_fanbox(p) {
  try {
    ...
    return {
      url: null,
      html: `<div style="border-style:solid;border-width:0.5px;padding:0.5em;">fanbox post (${p}) may not exist</div>`,
    };
  }
}
...
async function blog_t(body) {
  ...
  for (let x = 0; x < body.blocks.length; ++x) {
    ...
    case "file": {
      ...
      ret.push(
        `<a href="${f.url}" download="${f.name}.${f.extension}">${f.name}.${f.extension}</a>`,
      );
      break;
    }
    case "embed":
