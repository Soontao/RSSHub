...
const decrypt_login_xyz = (username, decrypt_password, nonce) => {
  const str =
    "/nr/user/login/loginByAccount?AppKey=joker&account=" +
    username +
    "&password=" +
    decrypt_password +
    "&state=1&nonce=" +
...
const decrypt_wechat_detail_xyz = (uid, nonce) => {
  const str =
    "/xdnphb/detail/v1/rank/article/lists?AppKey=joker&account=" +
    uid +
    "&nonce=" +
    nonce;
  return md5(str);
...
const decrypt_douyin_account_xyz = (nonce) => {
  const str =
    "/xdnphb/nr/cloud/douyin/detail/accountInfoAll?AppKey=joker&nonce=" + nonce;
  return md5(str);
};

const decrypt_douyin_detail_xyz = (nonce) => {
  const str =
    "/xdnphb/nr/cloud/douyin/detail/aweme?AppKey=joker&nonce=" + nonce;
  return md5(str);
};
