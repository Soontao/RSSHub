...
module.exports = async (ctx) => {
  ...
  const ywUrl =
    type !== "tt"
    	? "https://news.jnu.edu.cn/jnyw/List/List_149.html"
    	: "https://news.jnu.edu.cn/jnyw/jdtt/";
  // 发起 HTTP GET 请求
    ...
    const list = $("ul[class=newsConList] > li")
      .map(function () {
        const info = {
          ...
          link: url.resolve(
            ywUrl,
            $(this).find("div[class=title] > a").attr("href"),
          ),
          description: $(this).find("div[class=intro]").text(),
          pubDate: new Date(
            $(this).find("div[class=date]").text(),
          ).toUTCString(),
        };
        return info;
      })
    ...
    const list = $('ul[class="newsList infoList infoListA"] > li')
      .map(function () {
        const info = {
          ...
          description: "",
          pubDate: new Date(
            $(this).find("span[class=date]").text(),
          ).toUTCString(),
        };
        return info;
      })
