...
module.exports = async (ctx) => {
  ...
  const list =
    name === "all" ? $(".con_wap").find("li").get() : $("ul.caselist li").get();
  ...
  const out = await Promise.all(
    list.map(async (item) => {
      ...
      const other = await ctx.cache.tryGet(link, async () => {
        ...
        $(
          'img[src="/system/resource/images/fileTypeImages/icon_xls.gif"]',
        ).remove();
        $(
          'img[src="/system/resource/images/fileTypeImages/icon_doc.gif"]',
        ).remove();
        $(
          'img[src="/system/resource/images/fileTypeImages/icon_ppt.gif"]',
        ).remove();
        $(
          'img[src="/system/resource/images/fileTypeImages/icon_pdf.gif"]',
        ).remove();
  ...
  ctx.state.data = {
    title:
      name === "all" ? "团委 - 洛阳理工学院" : `${nameProps[name]} - 洛理团委`,
    link: urla,
