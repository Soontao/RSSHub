...
module.exports = async (ctx) => {
  ...
  const items = await Promise.all(
    [...links].map(async ({ title, link }) => {
      ...
      const newsContent = cheerio.load(response.data)(
        "form[name=_newscontent_fromname]",
      );
      const dateP = newsContent
        .find("h1", "div[class=cont]")
        .nextAll()
        .filter("p");

      const date = dateP.text().match(/日期：(\d{4})年(\d\d)月(\d\d)日/);
      item.pubDate = `${date[1]}-${date[2]}-${date[3]}T00:00+08:00`;
      item.description = newsContent
        .find("div[class=v_news_content]", "div[class=contentd]")
        .html();
