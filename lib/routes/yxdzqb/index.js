...
module.exports = async (ctx) => {
  ...
  const out = list
    .map((index, item) => {
      ...
      const title = item
        .find("div table:nth-child(1) tr td:nth-child(1)")
        .text();
      const description =
        `<img src=${item.find("table.cell_tabs > tbody > tr > td:nth-child(1) > img").attr("src")}>` +
        item.find("div.collapse").html();
      const link = item
        .find("div.collapse table.cell_tabs > tbody > tr > td:nth-child(1) > a")
        .attr("href");
