...
const weiboUtils = {
  formatExtended: (ctx, status, params = {}, picsPrefixes = []) => {
    ...
    const mergedParams = {
      readable: fallback(
        params.readable,
        queryToBoolean(routeParams.readable),
        false,
      ),
      ...
      widthOfPics: fallback(
        params.widthOfPics,
        queryToInteger(routeParams.widthOfPics),
        -1,
      ),
    ...
    htmlNewLineUnreplaced = htmlNewLineUnreplaced.replace(
      /<span class="url-icon"><img.*?alt="?(.*?)"? [\s\S]*?\/><\/span>/g,
      "$1",
    );
    // 去掉外部链接的图标
    htmlNewLineUnreplaced = htmlNewLineUnreplaced.replace(
