...
module.exports = async (ctx) => {
  ...
  if (token) {
    const response = await got.get(
      `https://api.weibo.com/2/statuses/home_timeline.json?access_token=${token}&count=100&feature=${feature}`,
    );
    // 检查token失效
    if (response.data.error !== undefined) {
      const {
        app_key = "",
        redirect_url = ctx.request.origin + "/weibo/timeline/0",
      } = config.weibo;
      ...
      ctx.redirect(
        `https://api.weibo.com/oauth2/authorize?client_id=${app_key}&redirect_uri=${redirect_url}`,
      );
    }
    // 获取所有表情
    const weiboEmotion = await ctx.cache.tryGet("weiboEmotion", async () => {
      ...
      await Promise.all(
        ["face", "ani", "cartoon"].map(async (type) => {
          try {
            const rep = await got.get(
              `https://api.weibo.com/2/emotions.json?access_token=${token}&type=${type}`,
            );
    ...
    const data = await Promise.all(
      response.data.statuses.map(async (item) => {
        const realContent = await ctx.cache.tryGet(
          "weibotimelineurl" + item.user.id + item.id,
          async () => {
            const res = [];
            const videoList = [];
            let picUrlList = [];
            ...
            try {
              const rep = await got.get(
                `https://m.weibo.cn/${item.user.id}/${item.id}`,
              );
              ...
              if (res.length > 1) {
                const sub_rep = await got.get(
                  `https://m.weibo.cn/${item.retweeted_status.user.id}/${item.retweeted_status.id}`,
                );
                sub_rep.body.replace(
                  /"text": "([\s\S]*?)",/g,
                  function (s, value) {
              ...
              res.forEach(function (content, index) {
                ...
                if (content.length > 0) {
                  emotion.forEach(function (item) {
                    res[index] = content.replace(
                      new RegExp(`\\${item}`, "g"),
                      `<img alt="${item}" src="${weiboEmotion[item]}" style="width:1em; height:1em;" referrerpolicy="no-referrer">`,
                    );
                  });
                }
              });
            ...
            try {
              ...
              content.replace(
                /<span class='url-icon'>([\s\S]*?)<\/span>/g,
                function (s, value) {
    ...
    if (code) {
      const rep = await got.post(
        `https://api.weibo.com/oauth2/access_token?client_id=${app_key}&client_secret=${app_secret}&code=${code}&redirect_uri=${redirect_url}&grant_type=authorization_code`,
      );
