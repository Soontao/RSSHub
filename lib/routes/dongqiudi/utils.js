  ...
  content("div.video").each((i, v) => {
    ...
    if (link.host === "m.miguvideo.com") {
      ...
      case "qiniu":
        ...
        content(
          `<video width="100%" controls="controls"> <source src="${link}" type="video/mp4"> Your RSS reader does not support video playback. </video>`,
        ).insertAfter(v);
        content(v).remove();
        break;
      case "youku":
        content(
          `<iframe height='100%' width='100%' src='${link}' frameborder=0 scrolling=no webkitallowfullscreen=true allowfullscreen=true></iframe>`,
        ).insertAfter(v);
        content(v).remove();
        break;
      default:
...
const ProcessHref = (content) => {
  content.each((j, y) => {
    if (y.attribs.href) {
      y.attribs.href = y.attribs.href.replace(
        "dongqiudi:///news",
        "https://www.dongqiudi.com/share/article",
      );
    }
  });
};
