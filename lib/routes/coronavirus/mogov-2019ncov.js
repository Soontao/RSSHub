...
module.exports = async (ctx) => {
  ...
  const items = response.data.json
    ...
    .map((item) => {
      ...
      const $ = cheerio.load(
        '<div id="root"><b id="department"></b><div id="photo"></div><div id="content"></div></div>',
      );
      if (item.contentPhotos) {
        $("#photo").append(
          item.contentPhotos.map((photo) => `<img src="${photo.url}"></img>`),
        );
      }
      ...
      const pubDate = new Date(
        item["broadcastTime_" + languageKey] + " +8",
      ).toUTCString();
