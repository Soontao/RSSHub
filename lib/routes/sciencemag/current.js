...
module.exports = async (ctx) => {
  ...
  const pageTitleName =
    pageCapture("head > title").text() || `Science (${journal})`;
  ...
  const list = [].concat.apply(
    ...
    sectionList.map((section) => {
      const sectionContent = pageCapture(
        `ul > li.issue-toc-section.issue-toc-section-${section}`,
      ).html();
  ...
  const out = await Promise.all(
    list.map(async (item) => {
      ...
      let author;
      const authorList = $(
        "span.highwire-citation-authors > span.highwire-citation-author",
      )
        .map((_, el) => $(el).text())
        .get();
      ...
      const time = new Date(
        $("p.highwire-cite-metadata > time").text(),
      ).toUTCString();
