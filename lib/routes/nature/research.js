...
module.exports = async (ctx) => {
  ...
  const pageDescription =
    pageCapture('meta[name="description"]').attr("content") ||
    "Nature, a nature research journal";
  const pageTitleName =
    pageCapture('meta[name="WT.cg_n"]').attr("content") ||
    `Nature (${journal})`;
  const pageTitleSub =
    pageCapture('meta[name="WT.cg_s"]').attr("content") || "Latest Research";
  ...
  const items = await Promise.all(
    list.map(async (el) => {
      ...
      const abs = itemCapture(
        "div#Abs1-content.c-article-section__content > p",
      ).html();
      let absContents = "";
      if (abs !== null) {
        absContents = `
      ...
      const citation = itemCapture(
        "p.c-bibliographic-information__download-citation > a",
      ).attr("href");
      const citationContents =
        citation !== undefined
        	? `<a href="${baseURL}${citation}">Download citation</a>`
        	: "";
      ...
      const pdfContents =
        pdf !== undefined ? `<a href="${pdf}">Offical PDF</a>` : "";
      const linkContents =
        "<ul>" +
        [citationContents, doiContents, pdfContents]
        	.filter((x) => x !== "")
        	.map((x) => `<li class="rss-journal-tag-icon">${x}</li>`) +
        "</ul>";
      const infoContents = `
      ...
      const contentStyle = `
            ...
            `;
      const contents =
        briefContents + absContents + infoContents + contentStyle;
