...
module.exports = async (ctx) => {
  const keyword =
    ctx.params.keyword &&
    ctx.params.keyword !== "all" &&
    ctx.params.keyword !== "全部"
      ? ctx.params.keyword
      : "";
  ...
  ctx.state.data = {
    ...
    item: list
      .map((item) => {
        if (
          item.is_advertisement !== 0 ||
          !item.photo ||
          !item.photo.photo_info
        ) {
          return "";
        }
        return {
          ...
          description: item.photo.photo_info.image_list
            .map(
              (image) =>
                `<img src="${image.ori_pic_url}" style="max-width: 100%;" />`,
            )
            .join(""),
          link: url,
