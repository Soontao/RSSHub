...
module.exports = async (ctx) => {
  ...
  const key =
    type === "day"
      ? ctx.params.key.slice(0, 4) +
        "/" +
        ctx.params.key.slice(4, 6) +
        "/" +
        ctx.params.key.slice(6, 8)
      : ctx.params.key || "";
  const link =
    "https://onejav.com" +
    `/${type === "day" ? "" : type}${type === "new" ? "" : "/" + key}`.replace(
      "//",
      "/",
    );
      ...
      list
        .map((index, item) => {
          ...
          return {
            title:
              "【" +
              item.find("div.card-content.is-flex > h5 > a").text() +
              "】" +
              item.find("div.card-content.is-flex > h5 > span").text(),
            ...
            enclosure_url:
              "https://onejav.com" +
              item.find("p.control.is-expanded > a").attr("href"),
