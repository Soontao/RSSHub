...
module.exports = async (ctx) => {
  ...
  const count = (
    await got.get(
      "https://sik.search.blue.cdtapps.com/gb/en/product-list-page?category=products&f-toggles2=new_product",
    )
  ).data.productListPage.productCount;

  const products = (
    await got.get(
      `http://sik.search.blue.cdtapps.com/gb/en/product-list-page/more-products?category=products&start=1&end=${count}`,
    )
