...
module.exports = async (ctx) => {
  ...
  const parseContent = (htmlString) => {
    ...
    const time =
      $(".topic-update-time").length === 0
        ? $(".topic-info [data-dynamic-time]")
        : $(".topic-update-time [data-dynamic-time]");
    const pub_date =
      time.length === 0 ? new Date() : new Date(time.text().trim());

    const images = $("img");
    for (let k = 0; k < images.length; k++) {
      $(images[k]).replaceWith(
        `<img src="${$(images[k]).attr("data-origin-url")}" />`,
      );
    }
