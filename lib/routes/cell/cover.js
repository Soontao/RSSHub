      ...
      const pageMeta = await ctx.cache.tryGet(
        ...
        async () => {
          const cookieData = await got
            ...
            .get(
              `https://secure.jbs.elsevierhealth.com/action/getSharedSiteSession?redirect=${encodeURI(pageURL)}`,
            )
            .then((response) => {
          ...
          const coverImg = url.resolve(
            "https://",
            $("#fullCover > img").attr("src"),
          );
          ...
          const issueNum = $(".toc-header__details span")
            ...
            .join(", ");
          const issueName =
            $('meta[name="name"]').attr("content").replace("Issue: ", "") ||
            "Cell ...";
