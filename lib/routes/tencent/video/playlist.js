...
module.exports = async (ctx) => {
  ...
  if (`${id[0]}` === "8") {
    vtype = 1;
    const episodes = await got.get(
      `https://s.video.qq.com/get_playsource?id=${id}&plat=2&type=${vtype}&data_type=2&video_type=3&range=1&plname=qq&otype=json&num_mod_cnt=20&callback=a&_t=${Date.now()}`,
    );
    const playList = JSON.parse(episodes.data.slice(2, -1)).playlist[0]
      .videoPlayList;
    ...
    const playList = JSON.parse(episodes.data.slice(2, -1)).PlaylistItem
      .videoPlayList;
