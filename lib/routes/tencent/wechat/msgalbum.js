...
module.exports = async (ctx) => {
  ...
  const list = $("li").get();
  const mptitle =
    $(".album__author-name").text() + "|" + $(".album__label-title").text();
  const articledata = await Promise.all(
    list.map(async (item) => {
      ...
      const content = $2("div#js_content.rich_media_content")
        .html()
        .replace(
          "iframe/preview.html?width=500&amp;height=375&amp;",
  ...
  ctx.state.data = {
    ...
    item: list.map((item, index) => ({
      ...
      description:
        $(item).find(".album__item-img").html() +
        `<br><br>${articledata[index].content}`,
      link: `${articledata[index].link}`,
      author: `${articledata[index].author}`,
      pubDate: dayjs
        .unix($(item).find(".js_article_create_time").text())
        .format(),
    })),
  };
};
