...
const apiUrl =
  "https://apps.game.qq.com/wmp/v3.1/?p0=18&p1=searchNewsKeywordsList&order=sIdxTime&r0=cors&type=iTarget&source=app_news_search&pagesize=12&page=1&id=";
...
const getPage = async (id, typeName) => {
  ...
  return await Promise.all(
    response.data.msg.result.map(async (item) => {
      let description = await got.get(
        `https://apps.game.qq.com/wmp/v3.1/public/searchNews.php?p0=18&source=web_pc&id=${item.iNewsId}`,
      );

      description = JSON.parse(
        description.data.match(/(?<=var searchObj=).*(?<!;)/g),
      );
