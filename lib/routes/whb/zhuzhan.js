...
module.exports = async (ctx) => {
  ...
  const parseContent = (htmlString) => {
    ...
    const matched_date = /(\d+)年(\d+)月(\d+)日 (\d+):(\d+):(\d+)/.exec(
      date_and_author,
    );
    const date = new Date(
      parseInt(matched_date[1]),
      parseInt(matched_date[2]) - 1,
      parseInt(matched_date[3]),
      parseInt(matched_date[4]),
      parseInt(matched_date[5]),
      parseInt(matched_date[6]),
    );
