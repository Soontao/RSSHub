...
module.exports = async (ctx) => {
  if (
    !config.fanfou ||
    !config.fanfou.consumer_key ||
    !config.fanfou.consumer_secret ||
    !config.fanfou.username ||
    !config.fanfou.password
  ) {
  ...
  const timeline = await fanfou.get(`/favorites/${encodeURIComponent(uid)}`, {
    id: uid,
    mode: "lite",
    format: "html",
  });
