...
module.exports = async (ctx) => {
  const typefilter = ctx.params.type
  ...
  const parseContent = (htmlString) => {
    ...
    const author = $(
      "#wrapper > div:nth-child(1) > form > div:nth-child(2) > table > tbody > tr:nth-child(1) > td.postauthor > cite > a",
    ).text();
  ...
  const fetch = async (pageindex) => {
    ...
    title =
      (ctx.params.type
      	? `[${$(".threadlist .headactions strong").text()}]`
      	: "") + $("title").text();
