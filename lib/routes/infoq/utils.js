...
function renderHtml(doc) {
    ...
    for (let i = 0; i < doc.content.length; i++) {
        let content = doc.content[i];
        let attrs = "";

        for (let attr in content.attrs) {
            if (attr === "style") {
                let styles = content.attrs[attr];
                let styleString = "";
        ...
        if (content.type === "paragraph") {
            ...
            if (content.content) {
                for (let j = 0; j < content.content.length; j++) {
                    let subContent = content.content[j];
                    ...
                    if (subContent.type === "text") {
                        ...
                        for (let linkAttr in subContent.attrs) {
                            linkAttrs += `${linkAttr}="${subContent.attrs[linkAttr]}" `;
                        }

                        html += `<a ${linkAttrs}>${subContent.text}</a>`;
                    }
                }
            }
            ...
            for (let imgAttr in content.attrs) {
    ...
    const items = await Promise.all(
        list.map(async (e) => {
            ...
            const single = await cache.tryGet(uuid, async () => {
                ...
                const html = renderHtml(content_response.data)
