...
module.exports = async (ctx) => {
  ...
  // 获取番剧名称
  const animeName = $(
    "body > div.container.clear > div.clear > div:nth-child(2) > div.detail.con24.clear > dl > dd > h1",
  ).text();

  // @param {*} list 用于存储通过 " h4 > a "获取到的节点
  const beforeFilterList = $(
    "div.swiper-container.xfswiper1 > div > div > ul > li > a",
  ).get(); // 获取到剧集列表
  const list = [];
