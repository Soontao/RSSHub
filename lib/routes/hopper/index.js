...
module.exports = async (ctx) => {
  ...
  const to =
    ctx.params.to !== undefined ? ctx.params.to.toUpperCase() : "Anywhere";
...
const formatDesc = (e) => {
  ...
  const description = `<table><tbody><tr><th align="left" style="border: 1px solid black;">From</th><th align="left" style="border: 1px solid black;">To</th><th align="left" style="border: 1px solid black;">Price</th></tr><tr><td style="border: 1px solid black;">
        ${origin}</td><td style="border: 1px solid black;">${destination}</td><td style="border: 1px solid black;">${price}</td></tr></tbody></table>${dayjs(departureDate).format("YYYY MMM DD")} &#9992; ${dayjs(
  returnDate,
).format("YYYY MMM DD")}`;
  return {
    title,
