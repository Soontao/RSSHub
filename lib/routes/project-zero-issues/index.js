...
module.exports = async (ctx) => {
  const csrfresponse = await got.get(
    "https://bugs.chromium.org/p/project-zero/issues/list?can=1&q=&sort=-id&colspec=ID%20Status%20Restrict%20Reported%20Vendor%20Product%20Finder%20Summary",
  );
  const token = csrfresponse.data.match(/(?<='token': ')[^'\s]+(?=')/g)[0];

  const formdata =
    '{"query":"","cannedQuery":1,"projectNames":["project-zero"],"pagination":{"maxItems":50,"start":0},"sortSpec":"-id"}';
