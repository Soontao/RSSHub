...
module.exports = async (ctx) => {
  const response = await got.get(
    `https://f-droid.org/en/packages/${ctx.params.app}/`,
  );
  ...
  $(".package-versions-list")
    ...
    .each(function () {
      ...
      item.pubDate = new Date(
        $(this).find(".package-version-header").text().split("Added on ")[1],
      ).toUTCString();
      item.description = [
        $(this).find(".package-version-download").html(),
        $(this).find(".package-version-requirement").html(),
        $(this).find(".package-version-source").html(),
      ].join("<br/>");
