...
module.exports = async (ctx) => {
  ...
  const articledata = await Promise.all(
    list.map(async (item) => {
      ...
      const youtube =
        $2(".mcdr.playlist-control a").attr("data-video-type") ===
        "video/youtube"
        	? $2(".mcdr.playlist-control a")
        		.attr("data-video-url")
        		.replace("https://www.youtube.com/watch?v=", "")
        	: "";
      const mp4 =
        $2(".mcdr.playlist-control a").attr("data-video-type") === "video/mp4"
        	? $2(".mcdr.playlist-control a").attr("data-video-url")
        	: "";
