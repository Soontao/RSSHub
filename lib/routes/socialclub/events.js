  ...
  ctx.state.data = {
    ...
    item:
      ...
      events.map((x) => ({
        ...
        description:
          (x.imgUrl ? `<img src="${x.imgUrl}" /><br>` : "") + x.description,
        pubDate: new Date(x.startDate).getTime() / 1000,
        link: x.linkToUrl
          ? `https://zh-cn.socialclub.rockstargames.com/${x.linkToUrl}/${x.urlHash}`
          : `https://zh-cn.socialclub.rockstargames.com/events/${x.urlHash}/${x.slug}/1`,
      })),
  };
};
