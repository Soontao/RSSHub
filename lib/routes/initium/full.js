...
module.exports = async (ctx) => {
  ...
  const name =
    response.data.name ||
    (response.data[model] && response.data[model].name) ||
    "追踪";
  // 从v1直升的channel和tags里面是digests，v2新增的author和follow出来都是results
  const articles = response.data.results
    ? response.data.results
    : response.data.digests;
  // 如果model=author，那就是avatar；否则都是cover，要么就没封面
  const image =
    response.data[model] &&
    (response.data[model].cover || response.data[model].avatar);
  ...
  const getFullText = async (slug) => {
    ...
    if (data.content) {
      content += data.content
        .replace('<figure class="advertisement"/><br/>', "")
        .replace(/<br><br>-----------<br>.*$/g, "");
  ...
  const items = await Promise.all(
    articles
      ...
      .map(async (item) => ({
        ...
        author:
          item.article.authors.length > 0
            ? item.article.authors.map((x) => x.name).toString()
            : item.article.byline,
        category: item.article.channels
          .filter((x) => !x.homepage)
          .map((x) => x.name),
        ...
        guid: (await getFullText(item.article.slug)).endsWith(
          "内容为空，请稍后再来",
        )
          ? item.article.uuid + "-I-am-empty"
          : item.article.uuid,
      })),
  );
