...
module.exports = async (ctx) => {
  ...
  ctx.state.data = {
    ...
    item: recentSubmissions.map((item) => ({
      title:
        item.question.questionFrontendId +
        " " +
        item.question.title +
        " " +
        item.question.translatedTitle,
      description:
        dic_status[item.status.toString()] +
        " " +
        dic_lang[item.lang.toString()],
      pubDate: new Date(parseInt(item.submitTime) * 1000)
        .toLocaleString()
        .replace(/:\d{1,2}$/, " "),
