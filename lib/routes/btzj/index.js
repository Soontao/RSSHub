...
async function load(link, type, ctx) {
  ...
  const other = {};
  const ela$ = $(
    "#body > div > table:nth-child(2) > tbody > tr:nth-child(1) > td.post_td > div.attachlist > table > tbody > tr:nth-child(3) > td:nth-child(1) > a",
  );
  const ela$1 = $(
    "#body > div > table:nth-child(2) > tbody > tr:nth-child(1) > td.post_td > div > div.attachlist > table > tbody > tr:nth-child(3) > td:nth-child(1) > a",
  );
  const content$ = $(".bg1.border.post");
  const downloadPrefix =
    type === "base"
    	? "torrent下载链接:<br/><br/>" + maps[type]
    	: "torrent下载链接:<br/><br/>";
  const downloada = type === "base" ? ela$.attr("href") : ela$1.attr("href");
  const downloadURL =
    downloada === undefined
    	? "无下载链接"
    	: downloada.replace("dialog", "download").toString();
  const content = content$.html().toString();
  other.des =
    downloadPrefix + downloadURL + "<br/><br/>" + "内容简介:<br/>" + content;
  other.pubDate =
    type === "base"
    	? new Date($(".bg2.border b:nth-child(2)").html()).toUTCString()
