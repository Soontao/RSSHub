...
const getHeaders = () => ({
  ...
  "X-App-Code": "2007081",
  "User-Agent":
    "Dalvik/2.1.0 (Linux; U; Android 10; Redmi K30 5G MIUI/V12.0.3.0.QGICMXM) (#Build; Redmi; Redmi K30 5G; QKQ1.191222.002 test-keys; 10) +CoolMarket/10.4-2007081",
});
...
const parseDongtai = async (item, ctx) => {
  ...
  if (item.issummary) {
    ...
    description = await ctx.cache.tryGet(itemUrl, async () => {
      ...
      const message =
        "<p>" + result.data.data.message.split("\n").join("<br>") + "</p>";
      const picArr = item.picArr
        .filter((i) => i)
        .map((i) => `<img src="${i}">`); // 若无图片，item.picArr=[""]
