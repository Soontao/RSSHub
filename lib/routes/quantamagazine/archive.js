...
module.exports = async (ctx) => {
  ...
  const out = await Promise.all(
    list.map(async (item) => {
      ...
      const disturbing = [
        ".hide-on-print",
        ".post__aside__pullquote",
        "aside.post__sidebar.hide",
      ];
      batchRemoveAll(capture, disturbing);

      let contents = capture("#postBody").html();
      if (contents !== null) {
        const latex = contents.replace(
          /\$latex([\s\S]+?)\$/g,
          '<img align="center" src="https://latex.codecogs.com/png.latex?$1"/>',
        );
        contents = latex.replace(
          /<div id=[\S\s]+?"src":"(https?:?[\s\S]+?)",[\S\s]+?"caption":"([\s\S]*?)",[\S\s]+?<\/div>?/g,
          function (omit, src, cap) {
            const imgUrl = src.replace(/\\([^un])/g, "$1");
            const img = "<img src=" + imgUrl + " />";

            const noBS = cap.replace(/\\([^un])/g, "$1");
            const removeNL = noBS.replace(/\\n/g, "");
            const caption = removeNL.replace(
              /\\u([0-9]{1,3}[a-z][0-9]?|[0-9]{4}?)/g,
              function (omit, s) {
