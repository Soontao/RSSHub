...
const parseStatuses = (data) =>
  data.map((item) => {
    const content = item.content
      ? item.content.replace(/<span.*?>|<\/span.*?>/gm, "")
      : "";
...
async function getAccountIdByAcct(acct, ctx) {
  ...
  if (
    !(
      mastodonConfig.apiHost &&
      mastodonConfig.accessToken &&
      mastodonConfig.acctDomain
    )
  ) {
  ...
  const account_id = await ctx.cache.tryGet(cacheUid, async () => {
    ...
    const accountData = search_response.data.accounts.filter(
      (item) => item.acct === acctOnServer,
    );
