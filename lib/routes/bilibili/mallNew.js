...
module.exports = async (ctx) => {
  ...
  const response = await got({
    ...
    headers: {
      Referer:
        "https://mall.bilibili.com/newdate.html?noTitleBar=1&page=new&from=new_product&loadingShow=1",
    },
  });
  ...
  const type = response.data.data.vo.cateTabs.find(
    (item) => item.cateType === response.data.data.vo.currentCateType,
  ).cateName;
