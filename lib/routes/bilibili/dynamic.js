  ...
  ctx.state.data = {
    ...
    item: data.map((item) => {
      ...
      const getDes = (data) =>
        ...
        (data.vest && data.vest.content) +
          (data.sketch &&
            `<br>${data.sketch.title}<br>${data.sketch.desc_text}`) ||
        data.intro ||
        "";
      const getOriginDes = (data) =>
        (data &&
          (data.apiSeasonInfo &&
            data.apiSeasonInfo.title &&
            `//转发自: ${data.apiSeasonInfo.title}`) +
            (data.index_title && `<br>${data.index_title}`)) ||
        "";
      const getOriginName = (data) =>
        data.uname ||
        (data.author && data.author.name) ||
        data.upper ||
        (data.user && (data.user.uname || data.user.name)) ||
        (data.owner && data.owner.name) ||
        "";
      const getOriginTitle = (data) => (data.title ? `${data.title}<br>` : "");
      const getIframe = (data) =>
        !disableEmbed && data && data.aid
          ? `<br><br>${utils.iframe(data.aid)}<br>`
          : "";
