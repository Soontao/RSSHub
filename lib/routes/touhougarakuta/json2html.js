const indexInterview = (article, language) =>
  ogpImage(article) + article.index.about + index(article, language);

const indexComic = (article, language) =>
  ogpImage(article) +
  article.index.about +
  authors(article.index.authors) +
  `<img src="${article.indexCover.cover.sourceUrl}">` +
  index(article, language);

const indexNovel = (article, language) =>
  ogpImage(article) +
  article.index.about +
  authors(article.index.authors) +
  `<img src="${article.indexCover.cover.sourceUrl}">` +
...
const interview = (article, language) => {
  ...
  for (const block of article.interviewContents.block) {
    result +=
      `<h2 style="text-align: center;">${block.head}</h2>` + block.content;
  }
  return result;
};
...
const comic = (article, language) => {
  ...
  let result =
    ...
    (viewerurl
    	? `<p>Viewer: <a href="${viewerurl}">${viewerurl}</a></p>`
    	: "") +
    (niconicourl
    	? `<p>niconico: <a href="${niconicourl}">${niconicourl}</a></p>`
    	: "") +
...
const uncategorized = (article, language) =>
  ogpImage(article) +
  linkToIndex(article, language) +
  "<hr/>" +
  article.contents.texts;

/**
 * 生成超链接，并将 edit 替换为对应语言
 */
const href = (text, url, language) =>
  `<a href="${url.replace(/\/\/edit/, `//${language}`)}">${text}</a>`;
 ...
 */
const ogpImage = (article) =>
  `<img src="${article.ogpSettings.ogp.image.sourceUrl}"` +
  ` alt="${article.ogpSettings.ogp.image.altText}">`;
...
const index = (article, language) => {
  ...
  for (const item of article.index.item) {
    result +=
      `<hr/><p><b>${item.num}</b></p>` +
      (item.link
      	? href(item.title, item.link.url, language)
      	: `<p>${item.title}</p>`) +
