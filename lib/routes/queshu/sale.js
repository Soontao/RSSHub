const buildData = require("rss-libs/utils/common-config");
...
module.exports = async (ctx) => {
  ...
  ctx.state.data = await buildData({
    ...
    item: {
      item: "#tb_sale tr",
      title:
        "$('.news_sale_detail .sale_btn').text() + '：' + $('.news_sale_title a').text()",
      ...
      description: `$('.news_sale_detail .sale_btn').text() + '：' + $('.news_sale_title a').text()
            ...
            `,
      pubDate:
        "date($('.news_sale_detail .sale_time_end.inline_right').text())",
    },
  });
};
