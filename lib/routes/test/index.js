...
<img src="/DIYgod/RSSHub.jpg">`,
  ...
  } else if (ctx.params.id === "long-time-cache") {
    const itemsIdx = idx++;
    item.push({
      title: `Long Time Cache Title ${itemsIdx}`,
      description: `Long Time Cache Description ${itemsIdx}`,
      link: `Long Time Cache Link ${itemsIdx}`,
      pubDate: new Date(1546272000000),
      author: "Long Time Cache (Theo Sun)",
    });
  }
  ...
  for (let i = 1; i < 6; i++) {
    item.push({
      ...
      pubDate: new Date(
        (ctx.params.id === "current_time" ? new Date() : 1546272000000) -
          i * 10 * 1000,
      ).toUTCString(),
