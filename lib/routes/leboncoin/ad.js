    ...
    if (key === "locations") {
      filters.location.locations = value
        ...
        .map((l) => {
          ...
          if (l.startsWith("dn_")) {
            return {
              locationType: "department_near",
              department_id: l.slice(3),
            };
          }
...
// - all the images in thumbnail version, and link to large version
  [
    ...
    (
      (ad.images &&
        ad.images.urls_thumb &&
        ad.images.urls_thumb.map(
        	(url, i) =>
        		`<a href=${ad.images.urls_large[i]}><img src="${url}"></a>`,
        )) ||
      []
    ).join(""),
  ].join("<br>");
...
module.exports = async (ctx) => {
  ...
  ctx.state.data = {
    ...
    item: (response.data.ads || [])
      ...
      .map((ad) => ({
        title:
          (ad.price && ad.price[0] ? `${ad.price[0]}€ - ` : "") + ad.subject,
