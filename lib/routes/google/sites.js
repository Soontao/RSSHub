...
module.exports = async (ctx) => {
  ...
  const test_pages_count = init_response.data.match(
    /'sites-pagination-next-link-top', (\d+),/,
  );
  if (!test_pages_count) {
  ...
  const url =
    pages_count > 20
      ? `https://sites.google.com/site/${id}/system/app/pages/sitemap/list?offset=${pages_count - 20}`
      : init_url;
