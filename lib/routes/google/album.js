...
module.exports = async (ctx) => {
  ...
  const info =
    JSON.parse(
      response.data.match(
        /AF_initDataCallback.*?data:(\[[\s\S]{10,}?\])\n}\);/m,
      )[1],
    ) || [];
  ...
  ctx.state.data = {
    ...
    item: list.slice(0, 50).map((item) => {
      ...
      const description = isVideo
        ? `<video src="${item[1][0]}=m18" controls controlslist="nodownload" loop style="max-width: 100%;">`
        : `<img src="${item[1][0]}=w${item[1][1]}-h${item[1][2]}-no" style="max-width: 100%;">`;
      return {
