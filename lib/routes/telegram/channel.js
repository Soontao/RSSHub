...
module.exports = async (ctx) => {
  ...
  const resourceUrl = searchQuery
    ? `https://t.me/s/${username}?q=${encodeURIComponent(searchQuery)}`
  ...
  const feedTitle = searchQuery
    ? `「${searchQuery}」- ` +
      $(".tgme_channel_info_header_title").text() +
      " - Telegram 频道"
    : $(".tgme_channel_info_header_title").text() + " - Telegram 频道";
    ...
    item:
      ...
      list
      	.map((index, item) => {
      		...
      		/* "Forwarded From" tag */
      		const fwdFrom = () => {
      			const fwdFromNameObject = item.find(
      		...
      		const replyContent = () => {
      			if (item.find(".tgme_widget_message_reply").length !== 0) {
      				...
      				const replyAuthor = replyObject.find(
      					".tgme_widget_message_author_name",
      				).length
      					? replyObject.find(".tgme_widget_message_author_name").text()
      					: "";
      				const replyLink = replyObject.attr("href").length
      					? replyObject.attr("href")
      					: "";
      				const replyMetatext = replyObject.find(
      					".tgme_widget_message_metatext",
      				).length
      					? `<p><small>${replyObject.find(".tgme_widget_message_metatext").html()}</small></p>`
      					: "";
      				const replyText = replyObject.find(".tgme_widget_message_text")
      					.length
      					? `<p>${replyObject.find(".tgme_widget_message_text").html()}</p>`
      					: "";
  ...
  $(this)
    ...
    .match(/url\('(.*)'\)/)[1]
      		...
      		/* audio */
      		const messageAudioUrl = item.find("audio.tgme_widget_message_voice")
      			.length
      			? item.find("audio.tgme_widget_message_voice").attr("src")
      			: "";
      		const messageAudioText = item.find("audio.tgme_widget_message_voice")
      			.length
      			? `<audio src="${item.find("audio.tgme_widget_message_voice").attr("src")}"></audio>`
      			: "";
      		const messageAudioDuration = () => {
      			if (item.find(".tgme_widget_message_voice_duration").length) {
      				const durationInMmss = item
      					.find(".tgme_widget_message_voice_duration")
      					.text();
      		...
      		/* link preview */
      		const linkPreview = () => {
      			const linkPreviewSite = item.find(".link_preview_site_name").length
      				? `<b>${item.find(".link_preview_site_name").text()}</b><br>`
      				: "";
      			...
      			const linkPreviewDescription = item.find(
      				".link_preview_description",
      			).length
      				? `<p>${item.find(".link_preview_description").html()}</p>`
      				: "";
      			const linkPreviewImage =
              generateImg(".link_preview_image") +
              generateImg(".link_preview_right_image");
      		...
      		/* poll */
      		const pollQuestion = item.find(".tgme_widget_message_poll_question")
      			.length
      			? item.find(".tgme_widget_message_poll_question").text()
      			: "";

      		/* attachment */
      		const attachmentTitle = item.find(
      			".tgme_widget_message_document_title",
      		).length
      			? item.find(".tgme_widget_message_document_title").text() +
              " / " +
              item.find(".tgme_widget_message_document_extra").text()
      			: "";
      		...
      		/* message text & title */
      		const messageTextObject = item.find(
