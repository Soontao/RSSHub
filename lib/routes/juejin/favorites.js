...
module.exports = async (ctx) => {
  ...
  const response = await got({
    method: "get",
    url: `https://apinew.juejin.im/interact_api/v1/collectionSet/list?user_id=${userId}&cursor=0&limit=20`,
  });
  ...
  async function getPostId(item) {
    ...
    return (
      (collectPage.data.data &&
        collectPage.data.data.article_list.slice(0, 10)) ||
      []
    );
  }
