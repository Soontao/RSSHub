...
async function load(link, ctx) {
  return await ctx.cache.tryGet(link, async () => {
    ...
    // 获取标题
    const title = $(
      "body > div.container.mt40 > div > div > div.m-lg.text-center > div.p-sm",
    ).text();
    // 获取正文内容
    const introduce = $(
      "body > div.container.mt40 > div > div > div.m-lg.info-style-content",
    ).html();
    // eslint-disable-next-line no-useless-escape
    // 获取标题中的推送时间和作者等信息
    const temp = $(
      "body > div.container.mt40 > div > div > div.m-lg.text-center > div.m-b-sm",
    )
      .text()
      .replace(/[\r\n]/g, "");
    // 获取标题中的时间
    const dateTime =
      temp
        .substring(temp.indexOf("发布时间") + 5, temp.indexOf("信息来源"))
        .trim() + ":00";
    const pubDate = new Date(new Date(dateTime).getTime()).toUTCString();
    // 获取标题中的作者
    const author = temp
      .substring(temp.indexOf("信息来源") + 5, temp.indexOf("阅读次数"))
      .trim();
