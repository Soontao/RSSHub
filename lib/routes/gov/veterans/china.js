...
async function load(link, ctx) {
  return await ctx.cache.tryGet(link, async () => {
    ...
    // 临时参数，用以获取作者和推送时间
    const temp = $(
      "#main > div.outerlayer > div > div > div.article-info",
    ).text();
    // 格式化输出时间
    const dateTime =
      temp
...
module.exports = async (ctx) => {
  ...
  const page =
    type === "bnxx"
    	? "http://www.mva.gov.cn/sy/xx/bnxx/"
    	: type === "zcjd"
    		? "http://www.mva.gov.cn/jiedu/zcjd/"
    		: "http://www.mva.gov.cn/";
  ...
  const list =
    type === "index"
    	? $("#whTopSlide > div.bd > ul > li > a").get()
    	: $(
    		"#main > div.overview.container.clearfix > div.overview-right.fr > div > div.public_list_team > ul > li  > a",
    	).get();
