...
module.exports = async (ctx) => {
  ...
  const pubDate = moment(listRes.data.date, "YYYYMMDD").toISOString();
  ...
  const resultItem = await Promise.all(
    storyList.map(async (story) => {
      ...
      item.description = await ctx.cache.tryGet(key, async () => {
        ...
        return utils.ProcessImage(
          storyDetail.data.body.replace(/<div class="meta">([\s\S]*?)<\/div>/g, "<strong>$1</strong>").replace(/<\/?h2.*?>/g, "")
        );
      });
