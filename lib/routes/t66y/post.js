...
module.exports = async (ctx) => {
  ...
  const load = async (page) => {
    ...
    const title = /本頁主題: (.+)\n?/.exec(
      $("#main div.t .h:first-child").text(),
    )[1];
    const items = $(".t.t2")
      .get()
      .filter((item) => {
        const matchs = /uid=(\d+)/.exec(
          $(".tiptop>a:first-child", item).attr("href"),
        );
