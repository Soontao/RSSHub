...
module.exports = async (ctx) => {
  ...
  const res = await got_ins.get(
    url.resolve(base, `${section}${ctx.params.id}${typefilter}&search=today`),
  );
  ...
  const parseContent = (htmlString) => {
    ...
    let time = $(
      "#main > div:nth-child(4) > table > tbody > tr:nth-child(2) > th > div",
    ).text();
    ...
    time.setTime(
      time.getTime() +
        (sourceTimezoneOffset - time.getTimezoneOffset() / 60) * 60 * 60 * 1000,
    );

    const author = $(
      "#main > div:nth-child(4) > table > tbody > tr.tr1.do_not_catch > th:nth-child(1) > b",
    ).text();
    const content = $(
    ...
    for (let k = 0; k < images.length; k++) {
      $(images[k]).replaceWith(
        `<img src="${$(images[k]).attr("ess-data")}" />`,
      );
    }
    // Handle input tag
    images = $("input");
