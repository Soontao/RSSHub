...
module.exports = async (ctx) => {
  const channel = channelMap[ctx.params.channel]
    ? channelMap[ctx.params.channel]
    : ctx.params.channel;

  const response = await got.get(
    `https://www.guokr.com/apis/minisite/article.json?retrieve_type=by_wx&channel_key=${channel}&offset=0&limit=10`,
  );
