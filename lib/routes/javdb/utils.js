...
module.exports = {
  ...
  ProcessItems: async (ctx, currentUrl, title) => {
      ...
      list.map(
        async (item) =>
          await ctx.cache.tryGet(item.link, async () => {
            ...
            item.enclosure_url = content(
              "#magnets-content button[data-clipboard-text]",
            )
              .eq(0)
            ...
            item.description =
              content(".movie-info-panel .columns").html() +
    ...
    const subject =
      htmlTitle.indexOf("|") === -1 ? "" : htmlTitle.split("|")[0];
