...
module.exports = async (ctx) => {
  ...
  const api_url =
    "http://www.imaijia.com/xstb/front/authorArticleController.do?indexArticles&pageSize=20";
  ...
  const out = await Promise.all(
    list.map(async (info) => {
      ...
      const description = response.data.resList.articleDataF.content.replace(
        /src="/g,
        'src="http://www.imaijia.com',
      );
