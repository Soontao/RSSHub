...
module.exports = async (ctx) => {
  const getPageUrl = (tid, page = 1, hash = "") =>
    `https://nga.178.com/read.php?tid=${tid}&page=${page}&rand=${Math.random() * 1000}#${hash}`;
  ...
  const eraseQuote = (str) =>
    str
      .replace(/\[quote\](.+?)\[\/quote\]/, (m, b) => b)
      .replace(
        /\[b\].*?\[uid=\d+\](.+?)\[\/uid\].+?\[\/b\]/,
        (m, user) => `@${user} `,
      );
  const pipeImg = (str) =>
    str.replace(
      /\[img\](.+?)\[\/img\]/g,
      (m, src) =>
        `<img src='${src[0] === "." ? "https://img.nga.178.com/attachments" + src.substr(1) : src}'></img>`,
    );
  ...
  const items = $("#m_posts_c")
    ...
    .map((ind, post_) => {
      ...
      const pubDate = new Date(
        post.find(".postInfo > span").first().text(),
      ).toUTCString();
