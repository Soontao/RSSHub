...
module.exports = async (ctx) => {
  ...
  const responseraw = await got(url, {
    headers: { Referer: "http://bbs.tianya.cn" },
  });
  ...
  const items = await Promise.all(
    json.data.rows.map(async (ele) => {
      ...
      const commentData = await ctx.cache.tryGet(link, async () => {
        const commentResponse = await got(link, {
          headers: { Referer: "http://bbs.tianya.cn" },
        });
        const res = cheerio
          .load(
            commentResponse.data,
          )(`div[replyid='${ele.reply_id}'] div.atl-content div.bbs-content`)
          .html();
        return res;
      });
