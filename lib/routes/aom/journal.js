...
module.exports = async (ctx) => {
  ...
  if (!cfg) {
    throw Error(
      'Bad id. See <a href="https://docs.rsshub.app/journal.html#academy-of-management-journal">docs</a>',
    );
  }
  ...
  const list = $("h5")
    ...
    .map((_, item) => {
      item = $(item);
      const a =
        item.parent().get(0).tagName === "a" ? item.parent() : item.find("a");
      return {
        title: a.attr("title"),
      ...
      async (item) =>
        await ctx.cache.tryGet(item.link, async () => {
          ...
          item.pubDate = new Date(
            content('meta[name="dc.Date"]').attr("content"),
          ).toUTCString();
