      ...
      async (item) =>
        await ctx.cache.tryGet(item.link, async () => {
          ...
          const gallery = content(
            'section[data-test-id="GalleryImage"]',
          ).html();

          if (ctx.params.caty === "video") {
            item.description =
              content('div[data-test-id="EmbedIframe"]').html() +
              content('p[data-test-id="DekText"]').parent().html();
            item.pubDate = new Date(
              detailResponse.data.match(/pubDate":"(.*?)","channel/)[1],
            ).toUTCString();
          } else if (gallery !== null) {
            item.description = gallery;
            item.pubDate = new Date(
              detailResponse.data.match(/pubDate":"(.*?)","items/)[1],
            ).toUTCString();
          } else {
            item.description = article;
            item.pubDate = new Date(
              detailResponse.data.match(
                /pubDate":"(.*?)","publishedRevisions/,
              )[1],
            ).toUTCString();
          }
