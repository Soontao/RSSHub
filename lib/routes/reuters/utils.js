...
const ProcessFeed = async (link) => {
  ...
  const pubDate = $('meta[property="og:article:published_time"]')[0].attribs
    .content;
  ...
  if (
    cover !== "https://s4.reutersmedia.net/resources_v2/images/rcom-default.png"
  ) {
  ...
  const pageDataPattern = new RegExp(
    '(?<=<script type="text/javascript">window.RCOM_Data = ){.*}(?=;</script>)',
  );
