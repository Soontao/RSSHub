  ...
  $(".movie-box")
    ...
    .map(async ({ title, thumb, link, pubDate, aid }) => {
      ...
      if (detail.screenData) {
        detail.imgHTML = detail.screenData.map(
          (url) => `<img src="${url}"><br />`,
        );
      }
      return {
...
const getDetail = async (link, ctx) => {
  ...
  if (cache) {
    ...
    let filmId = /<span style="color:#CC0000;">([\s\S]*?)<\/span>/.exec(
      detailPage,
    );
    if (filmId) {
      filmId = filmId[1];
    ...
    let filmTime = /<span class="header">發行日期:<\/span>([\s\S]*?)<\/p>/.exec(
      detailPage,
    );
    if (filmTime) {
      filmTime = filmTime[1];
    ...
    let filmLast = /<span class="header">長度:<\/span>([\s\S]*?)<\/p>/.exec(
      detailPage,
    );
    if (filmLast) {
    ...
    let filmEstabName =
      /<span class="header">發行商:[\s\S]*?"(.*?)">(.*?)<\/a>/.exec(detailPage);
    if (filmEstabName) {
    ...
    let filmMakerbName =
      /<span class="header">製作商:[\s\S]*?"(.*?)">(.*?)<\/a>/.exec(detailPage);
    if (filmMakerbName) {
    ...
    let seriesName =
      /<span class="header">系列:[\s\S]*?"(.*?)">(.*?)<\/a>/.exec(detailPage);
    if (seriesName) {
    ...
    let directorName =
      /<span class="header">導演:[\s\S]*?"(.*?)">(.*?)<\/a>/.exec(detailPage);
    if (directorName) {
    ...
    const code =
      /<span class="header">識別碼:[\s\S]*?">([\s\S]*?)<\/span>/.exec(
        detailPage,
      )[1];
    // 影片截图
    const regScreenshot =
      /<a class="sample-box" href="(.*?)"[\s\S]*?<img src="(.*?)">/g;
    match = detailPage.match(regScreenshot);
    let screenData = [];
    if (match) {
      screenData = match.map(
        (i) =>
          /<a class="sample-box" href="(.*?)"[\s\S]*?<img src="(.*?)">/g.exec(
            i,
          )[1],
      );
    } else {
    ...
    if (match) {
      magnetlinks = match.map(function (str) {
        // magnet地址
        const mlink =
          /td width="70%" onclick="window.open\('(magnet.*?)',/.exec(str)[1];
        // console.log(str);
        // 提取a链接
        const mrow = str.match(/<a [\s\S]*?magnet[\s\S]*?">([\s\S]*?)</gm);
        // magnet名称
        const mname = /<a [\s\S]*?magnet[\s\S]*?">([\s\S]*?)</
          .exec(mrow[0])[1]
          .trim();
        // magnet大小
        const msize = /<a [\s\S]*?magnet[\s\S]*?">([\s\S]*?)</
          .exec(mrow[1])[1]
          .trim();
        // return "<a href=\""+mlink+"\">"+mname+" -- "+msize+ "<\/a><textarea readonly rows=\"5\" cols=\"50\">"+ mlink + "<\/textarea><br \/>";
        return (
          '<a href="' + mlink + '">' + mname + " -- " + msize + "</a><br />"
        );
      });
    } else {
      magnetlinks = null;
