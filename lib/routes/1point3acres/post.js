...
module.exports = async (ctx) => {
  ...
  category == "new"
    ? ((url =
        "https://api.1point3acres.com/api/threads?type=new&pg=1&ps=20&includes=last_reply"),
    (title = "最新帖子"))
    : ((url =
        "https://api.1point3acres.com/api/threads?type=hot&pg=1&ps=20&includes=last_reply"),
    (title = "最热帖子"));
  ...
  ctx.state.data = {
    ...
    item: data.map((item) => {
      // eslint-disable-next-line eqeqeq
      item.last_reply.quote == ""
        ? (description = item.last_reply.message_bbcode)
        : (description = item.last_reply.quote);
      return {
        title: `${item.subject}`,
