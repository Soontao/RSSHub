...
module.exports = async (ctx) => {
  ...
  $("div.FeedCard").each(function (index, item) {
    if (
      $(item).find("a[class^=Component-headline]").attr("href") !== undefined
    ) {
      list.push(item);
    }
  });
  ...
  const out = await Promise.all(
    list.map(async (article) => {
      const link = url.resolve(
        "https://apnews.com",
        $(article).find("a[class^=Component-headline]").attr("href"),
      );

      const [title, author, pubDate, description] = await ctx.cache.tryGet(
        link,
        async () => {
