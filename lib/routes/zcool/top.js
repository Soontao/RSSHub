...
module.exports = async (ctx) => {
  ...
  if (type === "article") {
    const articleResponse = await got({
      method: "get",
      url:
        "https://www.zcool.com.cn/top/article.do?rankType=8&rankId=" + rankId,
      header: {
        Referer: url,
      },
    });
  ...
  const result = await Promise.all(
    list.map(async (item) => {
      ...
      item.description =
        type !== "article"
        	? itemElement("div.work-content-wrap").html()
        	: itemElement("div.article-content-wraper").html();
