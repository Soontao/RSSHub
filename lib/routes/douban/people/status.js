...
function getContentByActivity(status) {
  ...
  default:
    ...
    if (status.card) {
      ...
      if (
        status.card.image &&
          (status.card.image.large || status.card.image.normal)
      ) {
    ...
    if (status.video_card) {
      const videoCover =
          status.video_card.video_info &&
          status.video_card.video_info.cover_url;
      const videoSrc =
          status.video_card.video_info &&
          status.video_card.video_info.video_url;
      description += `${videoSrc ? `<br><video src="${videoSrc}" ${videoCover ? `poster="${videoCover}"` : ""}></video>` : ""}<br><br>${
        status.video_card.title
          ? `<a href="${status.video_card.url}">${status.video_card.title}</a>`
          : ""
      }`;
      ...
      items
        ...
        .map((item) => {
          ...
          return {
            ...
            pubDate: new Date(
              Date.parse(item.status.create_time + " GMT+0800"),
            ).toUTCString(),
