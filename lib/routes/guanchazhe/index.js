...
const gczUserCommunityLinkReg = /https:\/\/user\.guancha\.cn\/main\/content\?id=[0-9]+/gm;
        ...
        items = await Promise.all(
            links.map((itemLink) =>
                ctx.cache.tryGet(itemLink, async () => {
                    ...
                    const [gczCommunityLink] = (itemRes.body.match(gczUserCommunityLinkReg) || []);
                    if (gczCommunityLink !== undefined) {
                        ...
                        rssPageItem.link = gczCommunityLink;

                    } else {
                        const $item = cheerio.load(itemRes.body);
                        $item('iframe').removeAttr('width');
                        $item('iframe').removeAttr('style');
                        rssPageItem.title = $item('ul > li.left.left-main > h3').text();
                        rssPageItem.description = $item('div.content.all-txt').html();
                        try {
                            const pubDateString = $item($item('.time.fix span:nth-child(1)').get(0)).text();
                            rssPageItem.pubDate = moment(pubDateString, DATE_FORMAT).toDate();
                        } finally {
                            // do nothing
                        }
                    }
