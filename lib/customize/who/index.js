...
const endpoint = async (ctx) => {
  const url =
    "https://www.who.int/api/hubs/newsitems?sf_provider=OpenAccessDataProvider&sf_culture=zh&$orderby=PublicationDateAndTime%20desc&%24format=json&$top=10";
      ...
      return ctx.cache.tryGet(link, async () =>
        sem.use(async () => {
          ...
          return {
            ...
            author: $item(
              $item(".sf-item-header-wrapper .sf-tags-list-item").get(0),
            ).text(),
          };
        }),
      );
