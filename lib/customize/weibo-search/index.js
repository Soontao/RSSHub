  ...
  const resultItem = await Promise.all(
    flatten(responses.map((response) => response.data.data.cards))
      .filter(
        (item) =>
          item.card_type === 9 &&
          item.mblog !== undefined &&
          item.mblog.retweeted_status === undefined,
      )
      .map(async (item) => {
        ...
        if (isDataOK) {
          ...
          if (item.mblog.retweeted_status && status.retweeted_status) {
            item.mblog.retweeted_status.created_at =
        ...
        if (retweet && retweet.isLongText) {
          const retweetData = await weiboUtils.getShowData(
            retweet.user.id,
            retweet.bid,
          );
        ...
        if (item.mblog.isLongText) {
          description = await ctx.cache.tryGet(
            `weibo:extend:${item.mblog.bid}`,
            async () => {
        ...
        const pubDate = isDataOK
          ? new Date(status.created_at).toUTCString()
          : date(item.mblog.created_at, 8);
