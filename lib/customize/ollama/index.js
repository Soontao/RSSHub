...
const { createGenericEndpoint, linkExtractors, htmlToMarkdown, markdownToHtml } = require("@/utils/common-utils");
...
const endpoint = createGenericEndpoint({
  ...
  contentExtractor: async ($) => {
    ...
    const description = [
      markdownToHtml([await translate(htmlToMarkdown(rawContent).slice(0, 250)), "原文:"].join("\n\n")),
      rawContent,
    ].join("\n");
