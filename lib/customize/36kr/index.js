        ...
        ctx.cache.tryGet(item.link, () =>
          sem.use(async () => {
            try {
              ...
              const encryptedState = contentResponse.data.match(
                /initialState={"state":"(.*)","isEncrypt":true}/,
              )?.[1];
              ...
              if (encryptedState !== undefined) {
                ...
                item.title =
                  state?.articleDetail?.articleDetailData?.data?.summary;
                item.description =
                  state?.articleDetail?.articleDetailData?.data?.widgetContent;
                item.author =
                  state?.articleDetail?.articleDetailData?.data?.author;
                item.pubDate = new Date(
                  state?.articleDetail?.articleDetailData?.data?.publishTime,
                );
              } else {
                // old
                const content = cheerio.load(contentResponse.data);
                item.description = content(
                  "div.common-width.content.articleDetailContent.kr-rich-text-wrapper",
                ).html();
