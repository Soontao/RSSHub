...
const endpoint = async (ctx) => {
    ...
    const items = await Promise.all(
        list.map(
            async (item) =>
                await ctx.cache.tryGet(item.link, async () => sem.use(async () => {
                    try {
                        ...
                        if (content('#ne_wrap').attr('data-publishtime')) {
                            ...
                            item.pubDate = new Date(content('#ne_wrap').attr('data-publishtime') + ' GMT+8');
                            ...
                            if (content('div.headline span').text()) {
                                item.pubDate = new Date(content('div.headline span').text() + ' GMT+8');
                            }
                        ...
                        item.author = content('#ne_article_source').text() || content('#cc_side_con > h2').text();
