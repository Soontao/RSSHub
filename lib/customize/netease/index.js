...
const endpoint = async (ctx) => {
  ...
  if (!cfg) {
    throw Error(
      'Bad category. See <a href="https://docs.rsshub.app/new-media.html#wang-yi-xin-wen-pai-hang-bang">docs</a>',
    );
  } else if (
    (ctx.params.category !== "whole" &&
      ctx.params.type === "click" &&
      ctx.params.time === "month") ||
    (ctx.params.category === "whole" &&
      ctx.params.type === "click" &&
      ctx.params.time === "hour") ||
    (ctx.params.type === "follow" && ctx.params.time === "hour")
  ) {
    throw Error(
      'Bad time range. See <a href="https://docs.rsshub.app/new-media.html#wang-yi-xin-wen-pai-hang-bang">docs</a>',
    );
  }

  const currentUrl =
    ctx.params.category === "money" ? cfg.link : url.resolve(rootUrl, cfg.link);
        ...
        await ctx.cache.tryGet(item.link, async () =>
          sem.use(async () => {
            try {
              ...
              let data = "";
              if (res.headers["content-type"].endsWith("GBK")) {
                data = iconv.decode(res.data, "gbk");
              } else {
                data = iconv.decode(res.data, "utf8");
              }
              const content = cheerio.load(data);

              html.removeComments(content);

              const title = content("h1").first().text();
              ...
              if (content("#ne_wrap").attr("data-publishtime")) {
                ...
                item.pubDate = new Date(
                  content("#ne_wrap").attr("data-publishtime") + " GMT+8",
                );
                ...
                if (content("div.headline span").text()) {
                  item.pubDate = new Date(
                    content("div.headline span").text() + " GMT+8",
                  );
                }
              } else if (
                content('meta[property="article:published_time"]').attr(
                  "content",
                )
              ) {
                ...
                if (content('meta[property="article:published_time"]')) {
                  item.pubDate = new Date(
                    content('meta[property="article:published_time"]').attr(
                      "content",
                    ),
              ...
              } else if (content("#ptime").length > 0) {
                item.pubDate = new Date(content("#ptime").text() + "+8");
              }
              ...
              if (content(".pub_time").text()) {
                item.pubDate = new Date(content(".pub_time").text());
              }

              item.author =
                content("#ne_article_source").text() ||
                content("#cc_side_con > h2").text();
...
endpoint.path = "/netease/news/rank/:category?/:type?/:time?";

endpoint.name = "网易新闻";

endpoint.examples = ["/netease/news/rank/whole/click/day"];
