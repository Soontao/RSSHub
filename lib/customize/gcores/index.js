...
const fetchItem = concurrency.limit(async (itemPath, cache) => {
  ...
  return cache.tryGet(itemUrl, async () => {
    ...
    const single = {
      ...
      link: itemUrl,
      pubDate: moment
        .parseZone(apiRes.data.data.attributes["published-at"], DATE_FORMAT)
        .toDate(),
      guid: itemUrl,
      author: apiRes.data.included
        .map((include) => include.attributes.nickname)
        .join(", "),
    };
...
const endpoint = async (ctx) => {
  ...
  const out = await Promise.all(list.map((item) => fetchItem(item, ctx.cache)));
