...
const endpoint = async (ctx) => {
  ...
  const out = await Promise.all(
    list.map(async (i) => {
      ...
      time.setTime(
        time.getTime() +
          (sourceTimezoneOffset - time.getTimezoneOffset() / 60) *
            60 *
            60 *
            1000,
      );
      ...
      const single = {
        ...
        link: itemUrl,
        guid: itemUrl,
        author: itemElement(".news_warp_top li .name").text(),
      };
...
endpoint.path = "/3dm/news";

endpoint.name = "3DM 新闻";

endpoint.examples = ["/3dm/news"];
