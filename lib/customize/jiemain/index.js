...
const endpoint = async (ctx) => {
    ...
    const title = '界面新闻';
    const links = uniq(Array.from($('a[content_type=\'article\']').map((_, item) => 'https://www.jiemian.com/article/' + $(item).attr('content_id') + '.html'))).slice(0, 15);
    ...
    const items = await Promise.allSettled(
        links.map((link) => ctx.cache.tryGet(
            ...
            async () => sem.use(async () => {
                ...
                if (itemResponse.redirectUrls.length > 0) {
