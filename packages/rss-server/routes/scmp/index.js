const parser = require("rss-libs/utils/rss-parser");
    ...
    feed.items.map(
      async (item) =>
        await ctx.cache.tryGet(item.link, async () => {
          ...
          // Cover
          const cover = $(
            ".article-images > amp-carousel > .i-amphtml-slides-container >.i-amphtml-slide-item > amp-img > img",
          );

          if (cover.length > 0) {
            $(`<img src=${cover[0].attribs.content}>`).insertBefore(
              content[0].childNodes[0],
            );
            $(cover).remove();
          }
          ...
          const publishedAt = $('meta[itemprop="datePublished"]').attr(
            "content",
          );
          ...
          content.find("amp-img").each((i, e) => {
            const img = $(
              `<img width="${e.attribs.width}" height="${e.attribs.height}" src="${e.attribs.src}" alt="${e.attribs.alt}">`,
            );
          ...
          content.find("div.video-wrapper > amp-iframe").each((i, e) => {
            const iframe = $(
              `<iframe width="${e.attribs.width}" height="${e.attribs.height}" src="${e.attribs.src}">`,
            );
            $(iframe).insertBefore(e);
            $(e).remove();
          });
