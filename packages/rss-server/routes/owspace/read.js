  ...
  if (parseInt(item.model) === 4) {
    const result = {
      title:
        ...
        "年" +
        parseInt(item.title.substring(4, 6)) +
        "月" +
        parseInt(item.title.substring(6, 8)) +
        "日",
      link: item.html5,
...
module.exports = async (ctx) => {
  const selectedType =
    ctx.params.type !== undefined ? parseInt(ctx.params.type) : 0;
  ...
  const responseItemArray = response.data.datas.filter(
    (item) => parseInt(item.model) !== 5,
  );

  const feedItemArray = await Promise.all(
    responseItemArray.map(handleItem(ctx)),
  );
