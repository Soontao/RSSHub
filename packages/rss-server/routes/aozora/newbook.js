...
const got = require("rss-libs/utils/got"); // get web content
...
module.exports = async (ctx) => {
  ...
  for (let i = 0; i < count; ++i) {
    ...
    const pub_date_raw = $(
      'table[summary="底本データ"] > tbody > tr:nth-child(3) > td:nth-child(2)',
    ).text();
    const pub_date_num = pub_date_raw
      .replace(/（.*）|日/g, "")
      .replace(/[年月]/g, "-");
    const pub_date = new Date(pub_date_num).toUTCString();
    const full_text_relative_link = $(
      "table.download > tbody > tr:nth-child(3) > td:nth-child(3) > a",
    ).attr("href");
    const full_text_link = link_dir + full_text_relative_link;
    const full_text_link_html =
      '<a href="' + full_text_link + '">いますぐXHTML版で読む</a><br>';
