...
module.exports = async (ctx) => {
  const region =
    ctx.params.region === "en" ? "" : ctx.params.region.toLowerCase() + ".";
  ...
  const filteredItems = feed.items.filter(
    (item) =>
      !item.link.includes("promotions") && item.link.includes("yahoo.com"),
  );
  const items = await Promise.all(
    filteredItems.map(async (item) => {
      const description = await ctx.cache.tryGet(item.link, async () => {
        ...
        const $descriptionRoot = cheerio.load(
          '<div><div class="img-root"></div><div class="content-root"></div></div>',
        );
        $descriptionRoot("div.img-root").append($("article img"));
        $descriptionRoot("div.content-root").append(
          $("article p").removeAttr("content"),
        );
