...
module.exports = async (ctx) => {
  ...
  const url = keyword
    ? `https://api.avgle.com/v1/search/${keyword}/0`
    : "https://api.avgle.com/v1/videos/0";
  ...
  const compact_number = (number) =>
    `${Intl.NumberFormat("en-US", { notation: "compact", compactDisplay: "short" }).format(number)}`;
  const like = (item) =>
    `[${Math.round((item.likes / (item.likes + item.dislikes)) * 100)}%/${compact_number(item.likes + item.dislikes)}/${compact_number(item.viewnumber)}]`;

  ctx.state.data = {
    title: `Avgle ${order}/${time}`,
    link: keyword
      ? `https://avgle.com/search/videos/${keyword}?o=${order}&t=${time}`
      : `https://avgle.com/videos?o=${order}&t=${time}`,
