...
module.exports = async (ctx) => {
  ...
  ctx.state.data = {
    ...
    item: response.data.data.hacktivity_items.edges.map((item) => {
      const author = item.node.reporter
        ? item.node.reporter.username
        : "Someone";
      const vendor = item.node.team.name;
      const state = item.node.report.substate.replace(/^[a-z]/g, (L) =>
        L.toUpperCase(),
      );
      const severity = item.node.severity_rating
        ? item.node.severity_rating.replace(/^[a-z]/g, (L) => L.toUpperCase())
        : "No Rating";
      const awarded = item.node.total_awarded_amount
        ? item.node.total_awarded_amount
        : 0;
      return {
        ...
        author: author,
        pubDate: new Date(
          item.node.latest_disclosable_activity_at,
        ).toUTCString(),
