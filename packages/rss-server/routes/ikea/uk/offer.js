  ...
  const loadProductsOnPage = (data) => {
    ...
    if (products.length > 0) {
      for (const p of products) {
        const price = $(p)
          .find(".range-revamp-compact-price-package__price-wrapper")
          .text();
        ...
        let title =
          $(p).find(".range-revamp-header-section__title--small").text() + " ";

        title +=
          $(p)
            .find(".range-revamp-header-section__description-text")
            .text()
            .trim() + " ";

        const previous = $(p)
          .find(".range-revamp-compact-price-package__previous-price-text")
          .next("span")
          .text();
        ...
        const link = $(p)
          .find(".range-revamp-product-compact__bottom-wrapper > a")
          .attr("href");
