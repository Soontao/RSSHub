...
module.exports = async (ctx) => {
  ...
  const list = $(
    "#ctl00_ctl00_ContentPlaceHolder2_mainbody_rightbody_listcontent_NewsList",
  )
    .find(".gridline")
    .slice(0, 6)
    .map((index, item) => {
      ...
      return {
        ...
        pubDate: new Date(
          item.find(".gridlinedate").text().match(/\d+/g),
        ).toUTCString(),
      };
    })
    .get();
  ...
  const items = await Promise.all(
    [...list].map(async (item) => {
      ...
      const $ = cheerio.load(response.data);
      const article = $(
        "#ctl00_ctl00_ContentPlaceHolder2_mainbody_rightbody_listcontent_lmnr",
      );
