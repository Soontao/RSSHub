  ...
  const reqs = $(".project-file-name-container > a")
    ...
    .map(async (item) => {
      ...
      document.description =
        `${projectName} 的 ${document.author} 发布了新的文件: ${document.title}. ` +
        `</br> 支持的版本为: ${supportVersions}`;

      document.pubDate = new Date(
        Number($item(".standard-datetime").attr("data-epoch")) * 1000,
      ).toUTCString();
