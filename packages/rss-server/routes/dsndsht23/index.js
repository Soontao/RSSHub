...
module.exports = async (ctx) => {
  ...
  const subformId =
    subformName in forumIdMaps ? forumIdMaps[subformName] : subformName;
  ...
  const list = $("#threadlisttableid tbody[id^=normalthread]")
    ...
    .map(function () {
      const info = {
        title:
          "[" +
          $(this).find("th em a").text() +
          "] " +
          $(this).find("a.xst").text(),
  ...
  const out = await Promise.all(
    list.map(async (info) => {
      ...
      for (let k = 0; k < images.length; k++) {
        if (
          !$(images[k]).attr("file") ||
          $(images[k]).attr("file") === "undefined"
        ) {
          $(images[k]).replaceWith("");
        } else {
          $(images[k]).replaceWith(
            `<img src="${$(images[k]).attr("file")}" />`,
          );
        }
      }
      const description = (postMessage.html() || "抓取原帖失败").replace(
        /ignore_js_op/g,
        "div",
      );
