    ...
    pollution.length === 0
      ...
      : pollution.split(",").reduce((result, item) => {
        result += `${pollutionType[item].toUpperCase()}:<b>${
          typeof data.historic[pollutionType[item]] === "object"
            ? data.historic[pollutionType[item]][
              Object.keys(data.historic[pollutionType[item]])[0]
            ]
            : data.historic[pollutionType[item]][0]
        }</b><br>`;
        return result;
      }, "");
