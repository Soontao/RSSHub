...
const { fallback, queryToBoolean, queryToInteger } = require("rss-libs/utils/readable-social");
...
const weiboUtils = {
  formatExtended: (ctx, status, params = {}, picsPrefixes = []) => {
    ...
    htmlNewLineUnreplaced = htmlNewLineUnreplaced.replace(
      /<span class=["|']url-icon["|']>.*?<\/span><span class="surl-text">(.*?)<\/span>/g,
      "$1"
    );
    // 去掉乱七八糟的图标
  ...
  formatVideo: (itemDesc, status) => {
    ...
    if (pageInfo && pageInfo.type === "video") {
      ...
      const videoUrl = mediaInfo
        ? mediaInfo.stream_url_hd || mediaInfo.stream_url || mediaInfo.mp4_hd_url || mediaInfo.mp4_sd_url || mediaInfo.mp4_720p_mp4
        : "";
      if (videoUrl) {
