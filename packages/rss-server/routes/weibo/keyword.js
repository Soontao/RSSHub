...
const timezone = require("rss-libs/utils/timezone");
const { fallback, queryToBoolean } = require("rss-libs/utils/readable-social");
...
module.exports = async (ctx) => {
  ...
  ctx.state.data = {
    ...
    item: data.map((item) => {
      ...
      if (
        item.mblog.retweeted_status &&
        item.mblog.retweeted_status.created_at
      ) {
        item.mblog.retweeted_status.created_at = timezone(
          item.mblog.retweeted_status.created_at,
          +8,
        );
      }
