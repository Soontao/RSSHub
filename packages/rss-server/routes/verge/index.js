...
module.exports = async (ctx) => {
  ...
  const items = await Promise.all(
    feed.items.slice(0, 10).map(async (item) => {
      ...
      // 处理封面视频
      $(
        "div.l-col__main > div.c-video-embed, div.c-entry-hero > div.c-video-embed",
      ).each((i, e) => {
        const src = `https://volume.vox-cdn.com/embed/${e.attribs["data-volume-uuid"]}?autoplay=false`;

        $(
          `<iframe src="${src}" style="border: 0; top: 0; left: 0; width: 100%; height: 100%; position: absolute;" allowfullscreen scrolling="no"></iframe>`,
