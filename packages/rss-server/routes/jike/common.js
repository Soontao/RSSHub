  ...
  topicDataHanding: (data) =>
    data.posts.map((item) => {
      ...
      if (item.linkInfo) {
        ...
        if (audioObject) {
          const audioImage =
            audioObject.image.picUrl || audioObject.image.thumbnailUrl;
        ...
        if (videoObject) {
          const videoImage =
            videoObject.image.picUrl || videoObject.image.thumbnailUrl;
      ...
      if (item.pictures) {
        item.pictures.forEach((pic) => {
          if (pic.format === "gif") {
            ...
            const imgUrl = pic.picUrl.match(/\.[a-z0-9]+?\?imageMogr2/)
              ? pic.picUrl.split("?imageMogr2/")[0]
              : pic.picUrl.replace(/thumbnail\/.+/, "");
