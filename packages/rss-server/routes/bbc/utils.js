...
const ProcessVideo = ($, e, c) => {
  ...
  video.each((i, f) => {
    ...
    if (link.length > 0) {
      const url = JSON.parse(link[0].attribs["data-playable"]).settings
        .externalEmbedUrl;
...
const ProcessFeed = ($, link) => {
  ...
  // remove useless DOMs
  content
    .find("div.bbccom_slot, .off-screen, .embed-report-link")
    .each((i, e) => {
      $(e).remove();
    });
  ...
  if ($("#comp-media-player").length > 0) {
    ...
    const cover = $("#comp-media-player").find(
      "img.player-with-placeholder__image",
    );
  ...
  content.find("figure").each((i, e) => {
    ...
    ProcessVideo(
      $,
      e,
      "figure.media-with-caption > div.player-with-placeholder",
    );
