...
const { NotFoundError } = require("rss-libs/errors");
...
module.exports = async (ctx, next) => {
  ...
  if (ctx.state.data) {
    if (
      (!ctx.state.data.item || ctx.state.data.item.length === 0) &&
      !ctx.state.data.allowEmpty
    ) {
      throw new Error(
    ...
    ctx.state.data.title &&
      (ctx.state.data.title = he.decode(ctx.state.data.title + ""));
    ctx.state.data.description &&
      (ctx.state.data.description = he.decode(ctx.state.data.description + ""));

    // sort items
    ctx.state.data.item = ctx.state.data.item.sort(
    ...
    ctx.state.data.item.forEach((item) => {
      ...
      if (item.description) {
        ...
        item.description =
          he.decode($("body").html() + "") + (config.suffix || "");
      }
    });
    ...
    if (ctx.query) {
      // limit
      if (ctx.query.limit) {
        ctx.state.data.item = ctx.state.data.item.slice(
          0,
      ...
      if (ctx.query.filter) {
        ctx.state.data.item = ctx.state.data.item.filter((item) => {
          ...
          const isFilter =
            title.match(makeRegex(ctx.query.filter)) ||
            description.match(makeRegex(ctx.query.filter)) ||
            author.match(makeRegex(ctx.query.filter));
          return isFilter;
        });
      }
      ...
      if (
        ...
        (ctx.query.filter_title ||
          ctx.query.filter_description ||
          ctx.query.filter_author)
      ) {
        ctx.state.data.item = ctx.state.data.item.filter((item) => {
          const title = item.title || "";
          const description = item.description || title;
          ...
          ctx.query.filter_description &&
            (isFilter =
              isFilter &&
              description.match(makeRegex(ctx.query.filter_description)));
          ctx.query.filter_author &&
            (isFilter =
              isFilter && author.match(makeRegex(ctx.query.filter_author)));
          return isFilter;
        });
      }
      ...
      if (
        ctx.query.filterout ||
        ctx.query.filterout_title ||
        ctx.query.filterout_description ||
        ctx.query.filterout_author
      ) {
        ...
        ctx.state.data.item = ctx.state.data.item.filter((item) => {
          ...
          ctx.query.filterout_title &&
            (isFilter =
              isFilter && !title.match(makeRegex(ctx.query.filterout_title)));
          ctx.query.filterout_description &&
            (isFilter =
              isFilter &&
              !description.match(makeRegex(ctx.query.filterout_description)));
          ctx.query.filterout_author &&
            (isFilter =
              isFilter && !author.match(makeRegex(ctx.query.filterout_author)));
          return isFilter;
        });
      }
        ...
        ctx.state.data.item = ctx.state.data.item.filter(({ pubDate }) => {
          ...
          try {
            isFilter =
              ...
              parseInt(ctx.query.filter_time) * 1000;
          } catch (err) {
            // no-empty
          }
          return isFilter;
      ...
      if (ctx.query.scihub) {
        ctx.state.data.item.map((item) => {
          item.link = item.doi
            ? `https://sci-hub.tw/${item.doi}`
            : `https://sci-hub.tw/${item.link}`;
          return item;
        });
      }
    }
