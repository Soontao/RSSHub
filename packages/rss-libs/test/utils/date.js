...
describe("date", () => {
  ...
  it("D天前", async () => {
    expect(+new Date(parseDate("10天前"))).toBe(
      +date - 10 * 24 * 60 * 60 * 1000,
    );
  });

  it("M月前", async () => {
    expect(+new Date(parseDate("1月前"))).toBe(
      +date - 1 * 31 * 24 * 60 * 60 * 1000,
    );
  });

  it("Y年前", async () => {
    expect(+new Date(parseDate("1年前"))).toBe(
      +date - 1 * 365 * 24 * 60 * 60 * 1000,
    );
  });

  it("今天 H:m", async () => {
    expect(+new Date(parseDate("今天 08:00"))).toBe(
      +new Date("2019-1-1 08:00"),
    );
  });

  it("昨天 H:m", async () => {
    expect(+new Date(parseDate("昨天 20:00"))).toBe(
      +new Date("2018-12-31 20:00"),
    );
  });

  it("前天 H:m", async () => {
    expect(+new Date(parseDate("前天 20:00"))).toBe(
      +new Date("2018-12-30 20:00"),
    );
  });

  it("Y年M月D日H时", async () => {
    expect(+new Date(parseDate("2018年4月2日1时"))).toBe(
      +new Date("2018-4-2 01:00"),
    );
  });

  it("Y年M月D日", async () => {
    expect(+new Date(parseDate("2018年4月2日"))).toBe(
      +new Date("2018-4-2 00:00"),
    );
  });

  it("Y-M-D H:m", async () => {
    expect(+new Date(parseDate("2018-4-2 02:03"))).toBe(
      +new Date("2018-4-2 02:03"),
    );
  });
  ...
  it("Y/M/D H:m:s", async () => {
    expect(+new Date(parseDate("2018/4/2 02:03:04"))).toBe(
      +new Date("2018-4-2 02:03:04"),
    );
  });
  ...
  it("M月D日 H:m", async () => {
    expect(+new Date(parseDate("2月3日 02:03"))).toBe(
      +new Date("2019-2-3 02:03"),
    );
  });
