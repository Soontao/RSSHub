...
describe("dateParser", () => {
  ...
  test("UTCString", () => {
    expect(
      dateParser(
        dayjs.utc(now.toUTCString()).locale("en").format("YYYY-MM-DD HH:mm:ss"),
      ),
  ...
  test("ISO 8601", () => {
    expect(
      dateParser(
        dayjs(now.toUTCString()).locale("en").format("YYYY-MM-DDTHH:mm:ssZ"),
      ),
    ).toBe(now.toUTCString());
  });
  ...
  test("htmlOffset", () => {
    expect(
      dateParser(
        dayjs(now.toUTCString()).locale("en").format("YYYY-MM-DDTHH:mm:ss"),
        null,
        "en",
        serverOffset,
      ),
  ...
  // error handling
  test("error handling", () => {
    expect(+new Date(dateParser("10分钟前", null, "Klingon"))).toBe(
      +now - 10 * 60 * 1000,
    );
  });
});
